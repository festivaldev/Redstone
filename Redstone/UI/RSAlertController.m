...
- (void)addAction:(RSAlertAction*)action {
	...
	CGFloat buttonWidth = (self.view.frame.size.width - 60 - ((MAX(actions.count, 1) - 1)*5)) / MAX(actions.count, 1);
...
- (void)show {
	...
	[[[[RSCore sharedInstance] homeScreenController] alertControllers] addObject:self];
	[[[[RSCore sharedInstance] homeScreenController] view] addSubview:self.view];
...
- (void)dismiss {
	...
	dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.25 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
		...
		[[[[RSCore sharedInstance] homeScreenController] alertControllers] removeObject:self];
	});
}
